...
# --- attacco: scegli quale usare tra quelli implementati -------------------
class Attack(Enum):
    FGSM = "fgsm"
    PGD = "pgd"
    PGD_ADAPTIVE = "pgd_adaptive"

TRAIN_ATTACK = Attack.PGD.value
EVAL_ATTACK = Attack.PGD_ADAPTIVE.value
...
def _detector_grad_enabled(model):
    ...
    saved = [layer.detach for layer in layers]
    for layer in layers:
        layer.detach = False
    try:
        yield
...
# ===========================================================================
# Selezione dell'attacco tramite la costante globale ATTACK
# ===========================================================================
def generate_attack(model, x, y, eps, mask=None):
    """Genera il batch adversarial secondo la costante globale ATTACK
    ('fgsm' | 'pgd' | 'pgd_adaptive'). È il punto unico usato sia in training
    sia in valutazione: cambiando ATTACK cambia tutto in modo coerente."""
    ...
    if TRAIN_ATTACK == "pgd_adaptive":
        return pgd_adaptive(model, x, y, eps, steps=PGD_STEPS, alpha=PGD_ALPHA,
                            mask=mask, evade_weight=PGD_EVADE_WEIGHT)
    raise ValueError(f"ATTACK sconosciuto: {TRAIN_ATTACK!r} "
                     f"(valori validi: 'fgsm', 'pgd', 'pgd_adaptive')")
...
def train_epoch(model, loader, optimizer, eps, lambda_det=1.0,
    ...
    for x, y in loader:
        ...
        # 1) gemello adversarial del batch reale (attacco scelto in ATTACK)
        x_adv = generate_attack(model, x, y, eps, mask=attack_mask)
...
def evaluate(model, X_te, y_te, eps, attack_mask=None, device="cpu",
    ...
    for i in range(0, len(X_te), batch_size):
        ...
        x_adv = generate_attack(model, x, y, eps, mask=attack_mask)  # serve il gradiente
...
def select_threshold(model, X_val, y_val, eps, attack_mask=None, device="cpu",
                     batch_size=4096, grid=99):
    ...
    Gli adversarial di validation sono generati con l'attacco scelto in ATTACK,
    così la soglia è tarata contro la minaccia che ci interessa."""
    model.eval()
    sc_c, sc_a = [], []
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        # validation: metriche di task e detector contro l'attacco scelto.
    ...
    best_threshold, val_bal = select_threshold(
        model, X_val, y_val, eps=EPS, attack_mask=attack_mask, device=device
    )
    print(f"\nsoglia detector scelta su validation: {best_threshold:.3f} "
          f"(balanced acc validation = {val_bal:.4f})")

    # ---- valutazione sul test set ufficiale (con la soglia scelta) ---------
